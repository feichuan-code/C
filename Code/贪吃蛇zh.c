...
int main()
{
    ...
    {
        if(kbhit()) dir=getch();
        snake[1]=snake[3]+((dir=='a')?-2:(dir=='d')?2:0);
        snake[0]=snake[2]+((dir=='w')?-1:(dir=='s')?1:0);
        if(board[snake[0]][snake[1]]==1)
        {
            snake_num+=2;
            do{food[0]=rand()%29+1;food[1]=(rand()%29+1)*2;}while(board[food[0]][food[1]]!=' ');
            board[food[0]][food[1]]=1;
        }
        else if(board[snake[0]][snake[1]]=='\26') return printf("game over!\n");
        else board[snake[snake_num]][snake[snake_num+1]]=' ';
